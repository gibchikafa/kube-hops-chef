...
action :config do
  bash "apply-config" do
    ...
    environment ({ 'HOME' => ::Dir.home(new_resource.user) })
    code <<-EOH
      kubectl config set-credentials #{node['hopsmonitor']['user']} --client-certificate=#{x509_helper.get_crypto_dir(node['hopsmonitor']['user'])}/kube/hopsmon.cert.pem --client-key=#{x509_helper.get_crypto_dir(node['hopsmonitor']['user'])}/kube/hopsmon.key.pem
    EOH
  end
end
